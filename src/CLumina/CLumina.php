...
class CLumina implements ISingleton {
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= LUMINA_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = LUMINA_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $lu->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $lu
    ...
    $lu = &$this;
    // First the default Lumina themes/functions.php
    include(LUMINA_INSTALL_PATH . '/themes/functions.php');
  ...
  /**
   * Draw HTML for a menu defined in $lu->config['menus'].
   *
